...
const ModalProject = ({ title, image, text, technologies, libries, date, url, setShowModal }: IProject) => {
	...
	return (
		<>
			...
			<div className='fadeIn fixed z-[999] flex flex-col laptop:justify-center laptop:gap-x-[4rem] laptop:flex-row laptop:flex-wrap gap-10 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white w-[90%] max-w-[100rem] px-[2rem] py-[3rem] laptop:py-[5rem] rounded-lg laptop:h-[84rem] max-h-[90vh] ' >
				...
				<h2 className='text-[2rem] font-semibold text-center laptop:w-full laptop:text-[2.6rem]'>{title}</h2>
				<picture className='block laptop:h-full w-full laptop:w-[45%] rounded-md max-h-[45rem] tablet:overflow-hidden laptop:max-h-[90%]'>
					<img className='w-full h-full object-cover tablet:object-contain laptop:object-cover' src={image} alt={title} />
				</picture>
				<section className=' flex flex-col laptop:w-[40%] gap-2 overflow-y-auto py-5'>
