...
import type { ICategoriesProjects, IProject, ITechnologies } from '@interface/home';
        ...
        {projects.map((item) => {
          ...
          return (
            <article key={`${activeCategory}-${item.id}`} className="portfolioCard">
              <button
                type="button"
                className="portfolioCard__media"
                onClick={() => setSelectedProject(item)}
                aria-label={`Abrir detalles de ${item.title}`}
              >
                <img src={item.image} alt={item.title} />
                <span className="portfolioCard__overlay">Ver proyecto</span>
              </button>
